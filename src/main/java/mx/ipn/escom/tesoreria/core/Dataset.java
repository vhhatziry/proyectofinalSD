 ...
 * account's id is its position in the file counting from {@code idBase}
 * (normally 1), so the first data line becomes account {@code idBase}; the first
 * line must therefore be parsed, not skipped. The owner name is the three name
 * fields joined with spaces, and the balance is parsed from its decimal text into
 * integer cents through {@link Money#toCents(String)} so no floating point ever
 * touches a balance.
 */
public final class Dataset {
     ...
     *
     * @param ledger the ledger to populate
     * @param csv    path to the accounts CSV
     * @param idBase id assigned to the first account (rows are numbered from here)
     * @return the number of accounts loaded
     * @throws IOException if the file cannot be read
     */
    public static int loadInto(Ledger ledger, Path csv, int idBase) throws IOException {
        int id = idBase;
        int loaded = 0;
